...
def start_programm(frac):
    ...
    print("")
    mitarbeiter = sicherung_str("Wie heißt der Mitarbeiter? ")
    bruttogehalt = sicherung_float("Wie hoch ist dein Gehalt? ")
    vL_AG = sicherung_float("Wie hoch ist vermögenswirksame Leistungen des AG? ")
    vS = sicherung_float("Wie hoch ist das vermögenswirksame Sparen? ")
    question_1 = sicherung_entsch("Ist die Lohnsteuer gegeben? j=ja/n=nein ")
    KiSt = sicherung_float("Wie viel % Kirchensteuer gibt es? ")
    question_3 = sicherung_int("Von Welchem Jahr möchtst du die BBG wählen 2025/2026? ")
    Zusatzbeitrag = sicherung_float("Wie hoch ist der Zusatzbeitrag? ")
    question_2 = sicherung_int("Wie viele Kinder gibt es? ")
    vorschuss = sicherung_float("Wie viel Vorschuss gibt es? ")
    print("")
    ...
    if question_1 == "j":
        lohnsteuer = sicherung_float("Wie hoch ist die Lohnsteuer? ")
        KiSt_euro = lohnsteuer * KiSt / 100
    else:
        lohnsteuer = sicherung_float("Die Lohnsteuer aus Lohnsteuertabelle rauslesen. ")
        KiSt_euro = sicherung_float("KiSt aus Lohnsteuertabelle rauslesen. ")
    ...
    #Ausgaben
    print(mitarbeiter )
    dynammische_dezimal("Das Bruttogehalt", bruttogehalt, frac)
    dynammische_dezimal("Der AG-Anteil der vermögenswirksamen Leistungen", vL_AG, frac)
    dynammische_dezimal("Das Steuern und Sozialversicherungspflichtige Gehalt", st_sv_gehalt, frac)
    print("")
    dynammische_dezimal("Die Lohnsteuer", lohnsteuer, frac)
    dynammische_dezimal("Die Kirchensteuer", KiSt_euro, frac)
    print("")
    dynammische_dezimal("KV", KV, frac)
    dynammische_dezimal("RV", RV, frac)
    dynammische_dezimal("AV", AV, frac)
    dynammische_dezimal("PV", PV, frac)
    print("")
    dynammische_dezimal("Das Nettogehalt", nettogehalt, frac)
    dynammische_dezimal("Das vermögenswirksame Sparen", vS, frac)
    dynammische_dezimal("Der Vorschuss", vorschuss, frac)
    print("")
    dynammische_dezimal("Der Überweisungsbetrag", überweisungbetrag, frac)
