...
def dezimal_aendern(dezimal):
    dezimal_ändern = sicherung_entsch("Möchtest du Kommastellen ändern j oder nicht n? j = ja/ n = nein ")
    if dezimal_ändern == "j":
        dezimal = sicherung_int("Dezimal Kommastellen ")
    else:
        dezimal = 2
    print(f"Neu anzeigen mit {dezimal} Nachkommastellen. ")
    return dezimal
...
def sicherung_entsch(aufforderung):
    while True:
        try:
            ...
            print("Ein unerwarterter Fehler ist aufgetretten. ")
